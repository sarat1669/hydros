...
ErlDrvData bin_start(ErlDrvPort port, char *command);
static long do_bin_start(long port, char *buf);
static int bin_init(void);
void bin_stop(ErlDrvData drv_data);
void bin_erlang_read(ErlDrvData drv_data, char* buf, ErlDrvSizeT len);
static int do_bin_erlang_read(long port, char *buf, int count);
...
ErlDrvData bin_start(ErlDrvPort port, char *command) {
	return (ErlDrvData) do_bin_start((long) port, command);
}


static long do_bin_start(port,buf) 
long port;
char *buf;
{
...
void bin_erlang_read(ErlDrvData drv_data, char* buf, ErlDrvSizeT len) {
	do_bin_erlang_read((long) drv_data, buf, (int) len);	
}
 
static int do_bin_erlang_read(port,buf,count)
long port;
char *buf;
int count;

{
	...
	if (chdir(buf+1) == 0)     /* sucsess */
	    driver_output((ErlDrvPort)this_port,"y",1);
	else
	    driver_output((ErlDrvPort)this_port,"n",1);
	return 0;
    case 'f':
#ifdef MSDOS
	...
	rbuf[0] = 'y';
	driver_output((ErlDrvPort)this_port,rbuf,1+size);
	driver_free(rbuf);
	close(fd);
	return 0;
    case 'w':
	...
	return 0;
    default:
	driver_failure((ErlDrvPort)this_port,-1);
	return 0;
    }
}
