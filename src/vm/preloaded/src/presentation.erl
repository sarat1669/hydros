...
start() ->
	os_keyboard:add_exclusive_listener(spawn(fun() -> server() end)),
	ok.

server() -> server(slides()).
...
display(Title) when is_list(Title) ->
	os_unsafe:instruction(single_screen),
	io:format(lists:flatten([ "~n" || _ <- lists:seq(1, ?HEIGHT div 2) ])),
	display_line_centered(Title);
display({Title, Points, Fun}) when is_function(Fun) ->
	Fun(),
	display({Title, Points});
display({Title, Points}) ->
	os_unsafe:instruction(single_screen),
	io:format("~s~n__________~n", [Title]),
	lists:foreach(
		fun display_line/1,
		Points
	).
...
display_line(Line) ->
	lists:foreach(
		fun display_word/1,
		string:tokens(format_line(Line), " ")
	),
	io:format("~n").

display_word(Word) ->
	{_X, Y} = get_cursor(),
	if (?WIDTH - Y - 1) > length(Word) -> io:format("~s ", [Word]);
	true -> io:format("~n~s ", [Word])
	end.

format_line({Fmt, Args}) ->
	lists:flatten(io_lib:format(Fmt, Args));
format_line(Str) ->
	lists:flatten(io_lib:format("~s", [Str])).

display_line_centered(Str) ->
	io:format("~s~s~n",
		[
			[ $  || _ <- lists:seq(1, (?WIDTH div 2) - (length(Str) div 2)) ],
			Str
		]
	).
...
quit() ->
	os_keyboard:remove_listener(self()),
	os_unsafe:instruction(clear_screen),
	os_unsafe:instruction(multi_screen),
	ok.
