...
format(PID, Str) when is_pid(PID) -> format(PID, Str, []);
format(Format, Args) -> format(wm_terminal, Format, Args).
...
format(Terminal, Format, Args) ->
	format(Terminal, Format, Args, []).
format(Terminal, Format, Args, Opts) ->
